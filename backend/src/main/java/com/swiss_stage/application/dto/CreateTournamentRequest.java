 ...
 * のCreateTournamentRequest参照)。 このクロスフィールド制約はBean Validationでは表現しづらいため、application層
 * (TournamentService)でValidationExceptionとして検証する。
 */
public record CreateTournamentRequest(
    @NotBlank(message = "大会名は必須です") @Size(max = 100, message = "大会名は100文字以内で入力してください") String name,
