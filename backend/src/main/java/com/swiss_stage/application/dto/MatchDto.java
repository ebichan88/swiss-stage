...
/** 対局DTO。player2 が null なら不戦勝(BYE)。 group は帰属グループ(必須)。卓表示は「A-1」形式に使う。 */
public record MatchDto(
  ...
  public static MatchDto from(
      Match m, Map<ParticipantId, Participant> participants, Map<GroupId, Group> groups) {
    ...
    return new MatchDto(
        ...
        group == null ? new GroupDto(m.groupId().value(), "(不明なグループ)") : GroupDto.from(group),
