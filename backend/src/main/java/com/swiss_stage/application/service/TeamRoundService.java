...
public class TeamRoundService {
  ...
  List<TeamRoundDto> assembleRounds(TournamentId tournamentId) {
    ...
    Map<Integer, List<TeamMatch>> matchesByRound =
        teamMatchRepository.findAllByTournamentId(tournamentId).stream()
  ...
  public GeneratedTeamRoundDto generateNextRound(TournamentId tournamentId, String ownerSub) {
    ...
    GeneratedMatches generated =
        generateGrouped(groups, teams, previousMatches, nextRoundNumber, tournament.teamSize());
  ...
  private GeneratedMatches generateGrouped(
      List<Group> groups,
      List<Team> teams,
      List<TeamMatch> previousMatches,
      int roundNumber,
      int teamSize) {
    ...
    for (Group group : groups) {
      List<Team> members = teams.stream().filter(t -> group.id().equals(t.groupId())).toList();
      List<TeamMatch> groupPrevious =
      ...
      if (active.size() == 1) {
        ...
        boolean hadBye =
            groupPrevious.stream().anyMatch(m -> m.isBye() && m.team1Id().equals(only.id()));
        if (hadBye) {
  ...
  TeamMatchDto applyReport(
    ...
    if (request.boardResults().stream()
        .anyMatch(r -> r == MatchResult.NONE || r == MatchResult.BYE)) {
  ...
  private static TeamRoundDto toRoundDto(
      Round round, List<TeamMatch> matches, Map<TeamId, Team> teams, Map<GroupId, Group> groups) {
    List<TeamMatchDto> matchDtos =
        matches.stream()
            .sorted(
                Comparator.comparing((TeamMatch m) -> m.groupId().value())
