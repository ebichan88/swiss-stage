...
/**
 * 共有ページ集約(GET /api/v1/shared/{token})のインメモリキャッシュ (14_performance_optimization.md
 * §3)。ラウンド確定直後に参加者全員(最大300名)が 一斉に組み合わせ・順位表を見るスパイクを、DynamoDBへのQuery数回に抑えて吸収する。
 *
 * <p>整合性は「更新系操作での明示evict」で保ち、TTLは取りこぼしの安全網。 evictとgetの競合では古い値が最長TTLの間残り得るが、単一インスタンス・
