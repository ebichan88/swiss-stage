...
/** 棋力帯グループの管理(05_swiss_pairing_algorithm.md §2.4)。 定義・割当の変更はすべて PREPARING 中のみ。 */
@Service
public class GroupService {
  ...
  /**
   * 段級位で全ACTIVE参加者を一括振り分けし、更新後の参加者一覧を返す。 チームには棋力の概念がないため団体戦では提供しない(05_swiss_pairing_algorithm.md
   * §5.2)
   */
  ...
  private static void requireUniqueName(List<Group> groups, String name, GroupId self) {
    boolean duplicated =
        groups.stream().anyMatch(g -> g.name().equals(name) && !g.id().equals(self));
    if (duplicated) {
