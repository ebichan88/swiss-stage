 ...
 *
 * <ul>
 *   <li>ヘッダー行必須: {@code 氏名,所属,段級位}(3列)または {@code 氏名,所属,段級位,グループ}(4列)
 *   <li>グループ列は任意。名前の解決(定義済みグループとの突き合わせ)は呼び出し側で行う
 *   <li>文字コードは UTF-8 / Shift_JIS を自動判定(UTF-8として厳密にデコードできなければShift_JIS)
 *   <li>行エラーは全行検査してまとめて返す(1件でもエラーがあれば取り込まない)
 *   <li>行数上限500(13_security_design.md §4)
 * </ul>
 */
@Component
public class ParticipantCsvParser {
  ...
  public record Row(
      String name, String organization, Rank rank, String groupName, int lineNumber) {}
  ...
  private void parseLine(
      String line, int lineNumber, int columnCount, List<Row> rows, List<FieldErrorDto> errors) {
  ...
  private int validateHeader(String headerLine) {
    ...
    if (!valid) {
      throw invalid(
          List.of(new FieldErrorDto("1行目", "ヘッダー行は「氏名,所属,段級位」または「氏名,所属,段級位,グループ」である必要があります")));
    }
