...
public class TeamStandingService {
  ...
  List<GroupTeamStandingsDto> assembleStandings(TournamentId tournamentId) {
    ...
    for (Group group : groups) {
      List<Team> groupTeams = teams.stream().filter(t -> group.id().equals(t.groupId())).toList();
      List<TeamMatch> groupMatches =
          matches.stream().filter(m -> group.id().equals(m.groupId())).toList();
      result.add(
          new GroupTeamStandingsDto(GroupDto.from(group), calculate(groupTeams, groupMatches)));
    }
    return result;
  }

  private List<TeamStandingDto> calculate(List<Team> teams, List<TeamMatch> matches) {
    Map<TeamId, Team> byId =
