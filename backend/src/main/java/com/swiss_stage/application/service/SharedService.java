...
/**
 * 共有トークン経由のアクセス(13_security_design.md §2-3)。 無効・不明・非公開はすべて同じ
 * INVALID_SHARE_TOKEN(403)で返し、大会の存在を漏らさない。
 */
@Service
public class SharedService {
  ...
  /**
   * 共有ページ(S10)用の集約。shareToken・ownerSub は含めない。 ラウンド確定直後のアクセススパイクに備えキャッシュする(無効トークンはキャッシュされない)。
  ...
  public SharedTournamentDto getShared(String token) {
    return cache.get(
        ...
        t -> {
          ...
          SharedTournamentDto dto =
              tournament.isTeamCompetition()
