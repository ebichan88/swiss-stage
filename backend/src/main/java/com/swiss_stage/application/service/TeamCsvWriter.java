...
/**
 * 団体戦チーム+メンバーCSVのダウンロード出力(03_api_design.md §4-9)。
 * {@link TeamCsvParser} と対称のヘッダー(チーム名,氏名,段級位,ポジション,グループ)で
 * メンバー1人につき1行書き出す。UTF-8 BOM付き・CRLF区切り(Excel互換)。0件時はヘッダー行のみ返す。
...
public class TeamCsvWriter {
    ...
    private static final Map<Integer, String> POSITION_NAMES = Map.of(
            1, "主将", 2, "副将", 3, "三将", 4, "四将", 5, "五将");
        ...
        for (Team team : teams) {
            ...
            for (TeamMember member : team.members()) {
                sb.append(team.name()).append(',')
                        ...
                        .append(member.rank() == null ? "" : member.rank().displayName()).append(',')
                        .append(member.boardPosition() == null
                                ? "" : POSITION_NAMES.get(member.boardPosition()))
                        .append(',')
