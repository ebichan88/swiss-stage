 ...
 *
 * <ul>
 *   <li>ヘッダー行必須: {@code チーム名,氏名,段級位,ポジション}(4列)または {@code チーム名,氏名,段級位,ポジション,グループ}(5列)
 *   <li>連続する同一チーム名の行を1チームとして扱う(チーム名で離れた行を同一チームにはしない)
 *   <li>ポジション列: 主将/副将/三将/四将/五将(teamSize以内)。空欄は補欠
 *   <li>グループ列は任意。同一チームの最初の行の値のみ使う
 *   <li>文字コードは UTF-8 / Shift_JIS を自動判定。行エラーは全行検査してまとめて返す
 * </ul>
 */
@Component
public class TeamCsvParser {
  ...
  private static final Map<String, Integer> POSITION_LABELS =
      Map.of("主将", 1, "副将", 2, "三将", 3, "四将", 4, "五将", 5);
  ...
  public List<Row> parse(byte[] csv, int teamSize) {
    ...
    for (int i = 1; i < lines.size(); i++) {
      ...
      if (dataRowCount > MAX_ROWS) {
        throw invalid(List.of(new FieldErrorDto("file", "データ行が上限(" + MAX_ROWS + "行)を超えています")));
      }
  ...
  private void parseLine(
    ...
    if (!rankText.isEmpty()) {
      ...
      if (parsed.isEmpty()) {
        errors.add(new FieldErrorDto(field, "段級位「" + rankText + "」を解釈できません(例: 3級・初段・5段)"));
        return;
      }
      rank = parsed.get();
    }
    ...
    if (!positionText.isEmpty()) {
      ...
      if (position == null) {
        errors.add(new FieldErrorDto(field, "ポジション「" + positionText + "」を解釈できません(例: 主将・副将・三将)"));
        return;
      }
      if (position > teamSize) {
        errors.add(
            new FieldErrorDto(field, "ポジション「" + positionText + "」は" + teamSize + "人制では指定できません"));
        return;
      }
  ...
  private int validateHeader(String headerLine) {
    ...
    boolean valid =
        columns.length == BASE_HEADER.length || columns.length == BASE_HEADER.length + 1;
    ...
    if (!valid) {
      throw invalid(
          List.of(
              new FieldErrorDto(
                  "1行目", "ヘッダー行は「チーム名,氏名,段級位,ポジション」" + "または「チーム名,氏名,段級位,ポジション,グループ」である必要があります")));
    }
  ...
  private static ValidationException invalid(List<FieldErrorDto> details) {
    return new ValidationException(ErrorCode.CSV_INVALID_FORMAT, "CSVの内容に誤りがあります", details);
  }
}
