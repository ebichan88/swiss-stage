...
public class ParticipantService {
  ...
  public CsvImportResultDto importCsv(TournamentId tournamentId, String ownerSub, byte[] csv) {
    ...
    for (ParticipantCsvParser.Row row : rows) {
      GroupId groupId =
          row.groupName() == null ? defaultGroupId : groupsByName.get(row.groupName());
      participants.add(
          Participant.create(
    ...
    return new CsvImportResultDto(
        participants.size(), participants.stream().map(ParticipantDto::from).toList());
  }
  ...
  /** グループの解決。ユーザー入力をキー組み立てに使わないため、 大会のグループ一覧(最大10件)から突き合わせる */
