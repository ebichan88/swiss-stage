...
public class RoundService {
  ...
  List<RoundDto> assembleRounds(TournamentId tournamentId) {
    ...
    Map<Integer, List<Match>> matchesByRound =
        matchRepository.findAllByTournamentId(tournamentId).stream()
    ...
    return roundRepository.findAllByTournamentId(tournamentId).stream()
        ...
        .map(
            round ->
                toRoundDto(
                    round,
                    matchesByRound.getOrDefault(round.roundNumber(), List.of()),
                    participants,
                    groups))
        .toList();
  }
  ...
  public GeneratedRoundDto generateNextRound(TournamentId tournamentId, String ownerSub) {
    ...
    if (tournament.currentRound() >= 1) {
      ...
      if (!current.isConfirmed()) {
        throw new InvalidStateException("ラウンド" + current.roundNumber() + "を確定してから次のラウンドを生成してください");
      }
    }
  ...
  /** ラウンドの一括生成(05_swiss_pairing_algorithm.md §2.4)。 グループごとに独立ペアリングし、relaxations は全グループの和集合を返す。 */
  private GeneratedMatches generateGrouped(
    ...
    for (Group group : groups) {
      ...
      List<Match> groupPrevious =
          previousMatches.stream().filter(m -> group.id().equals(m.groupId())).toList();
      ...
      if (active.size() == 1) {
        ...
        boolean hadBye =
            groupPrevious.stream().anyMatch(m -> m.isBye() && m.player1Id().equals(only.id()));
        if (hadBye) {
  ...
  /**
   * ラウンド確定。結果が確定していない(result=NONE)対局が1件でも残っていると確定できない。 片方のみ申告・申告不一致でも運営者がresultを確定していなければブロックする
  ...
  public RoundDto confirm(TournamentId tournamentId, int roundNumber, String ownerSub) {
    ...
    if (undecided > 0) {
      throw new InvalidStateException("結果が未確定の対局が" + undecided + "件あります。全て入力してから確定してください");
    }
    Round confirmed;
    try {
   ...
   */
  MatchDto applyReport(
      TournamentId tournamentId, MatchId matchId, ReportMatchResultRequest request) {
    if (request.result() == MatchResult.NONE || request.result() == MatchResult.BYE) {
      throw new ValidationException("結果には勝敗・引き分け・両者敗けのいずれかを指定してください");
    }
    return editMatch(
  ...
  private MatchDto editMatch(
      TournamentId tournamentId, MatchId matchId, long expectedVersion, UnaryOperator<Match> edit) {
    Match match =
        matchRepository
  ...
  private static List<Match> toMatches(PairingResult pairing, int roundNumber, GroupId groupId) {
    ...
    for (PairingResult.Pair pair : pairing.pairs()) {
      matches.add(
          Match.pairOf(roundNumber, tableNumber++, pair.player1Id(), pair.player2Id(), groupId));
    }
    if (pairing.hasBye()) {
  ...
  private static RoundDto toRoundDto(
    ...
    List<MatchDto> matchDtos =
        matches.stream()
            .sorted(
                Comparator.comparing((Match m) -> m.groupId().value())
