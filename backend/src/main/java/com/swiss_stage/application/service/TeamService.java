...
public class TeamService {
  ...
  public TeamCsvImportResultDto importCsv(TournamentId tournamentId, String ownerSub, byte[] csv) {
    ...
    List<Group> groups = groupRepository.findAllByTournamentId(tournamentId);
    Map<String, GroupId> groupsByName =
    ...
    for (List<TeamCsvParser.Row> teamRows : csvParser.groupByTeam(rows)) {
      ...
      for (TeamCsvParser.Row row : teamRows) {
        team =
            team.withMember(TeamMember.create(row.memberName(), row.rank(), row.boardPosition()));
      }
  ...
  public CsvExport exportCsv(TournamentId tournamentId, String ownerSub) {
    ...
    List<Team> teams =
        teamRepository.findAllByTournamentId(tournamentId).stream()
            .sorted(Comparator.comparingInt(Team::entryOrder))
            .toList();
    Map<GroupId, String> groupNames =
        groupRepository.findAllByTournamentId(tournamentId).stream()
  ...
  public TeamDto update(
    ...
    if (request.groupId() != null) {
      ...
      groupId =
          resolveGroup(groupRepository.findAllByTournamentId(tournamentId), request.groupId()).id();
    }
    Team updated = team;
  ...
  public TeamDto addMember(
    ...
    Team team = load(tournamentId, teamId);
    Team updated =
        withDomainErrorsAsValidation(
            () ->
                team.withMember(
                    TeamMember.create(request.name(), request.rank(), request.boardPosition())));
  ...
  public TeamDto updateMember(
    ...
    Integer boardPosition =
        clearPosition
            ? null
            : request.boardPosition() != null ? request.boardPosition() : member.boardPosition();
    validateBoardPositionRange(boardPosition, tournament.teamSize());
    TeamMember replacement =
        new TeamMember(
            member.id(),
            ...
            boardPosition);
    Team updated =
        withDomainErrorsAsValidation(() -> team.withReplacedMember(memberId, replacement));
    validateReserveCount(updated, tournament.teamSize());
    teamRepository.save(tournamentId, updated);
    sharedViewCache.evict(tournamentId);
    return TeamDto.from(updated);
  ...
  private static void validateGroupNames(
    ...
    List<FieldErrorDto> errors =
        rows.stream()
            ...
            .toList();
    if (!errors.isEmpty()) {
      throw new ValidationException(ErrorCode.CSV_INVALID_FORMAT, "CSVの内容に誤りがあります", errors);
    }
  }
