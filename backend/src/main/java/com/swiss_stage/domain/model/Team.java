...
/**
 * 団体戦のチーム(Participantに相当するマッチング単位)。 groupId
 * は必須。個人戦と同じGroup機能をチーム単位で流用する(05_swiss_pairing_algorithm.md §5.2)。
...
public record Team(
  ...
  public Team {
    ...
    long distinctPositions =
        members.stream().map(TeamMember::boardPosition).filter(p -> p != null).distinct().count();
  ...
  public static Team create(String name, int entryOrder, GroupId groupId) {
    return new Team(
        TeamId.generate(), name, entryOrder, ParticipantStatus.ACTIVE, groupId, List.of());
  }
