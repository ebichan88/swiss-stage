...
/**
 * チームメンバー(値オブジェクト。Teamに埋め込み)。 boardPosition は 1..teamSize の必須ポジション(1=主将,2=副将,3=三将,4=四将,5=五将)、
 * またはnull(補欠)。boardPositionがteamSizeの範囲内か・重複がないか・補欠人数の上限は
 * Team単体では判定できない(teamSizeはTournamentが持つ)ため、application/domain層の
 * 開始前検証で行う(05_swiss_pairing_algorithm.md §5.1)。 対局結果はこのboardPositionではなくボード位置単位で記録され、実際に誰(正メンバーか
