...
/**
 * 対局。player2 が null の場合は不戦勝(BYE)を表す。 version は楽観ロック用(結果入力の競合検出。0 = 未保存)。 resultInputBy
 * は結果を確定した主体(監査用。未確定・BYEは null。 OWNER = 運営者が直接確定、SHARE_TOKEN = 両者の自己申告が一致して自動確定)。
 * player1/2ReportedResult はトークン経由の自己申告(NONE = 未申告)。確定済み(OWNER・
 * SHARE_TOKENいずれも)の対局は、以後の自己申告で記録だけ更新され、確定値(result)には 影響しない(確定が最終権威。食い違いの検知はDTO側で申告内容と確定結果を比較して行う)。
...
public record Match(
  ...
  public Match withResult(MatchResult newResult, ResultInputBy inputBy) {
    ...
    return new Match(
        ...
        newResult,
        inputBy,
        player1ReportedResult,
        player2ReportedResult,
        version,
        groupId);
  }
   ...
   * side側の申告を記録し、両者の申告が一致すればresultを自動確定する。 既に確定済み(resultが決定済み。運営者直接確定・自己申告一致の自動確定いずれも)の場合は
   * 申告の記録のみでresultは変えない(確定後の再申告でサイレントに結果が書き換わる/ 未確定へ巻き戻ることを防ぐ)。
   */
