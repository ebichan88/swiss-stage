 ...
 * (teamPoints)を都度比較して導出する(05_swiss_pairing_algorithm.md §5.3)。 version
 * はTeamMatch単位の楽観ロック(ボード配列をまとめて1回の更新で書き換える設計のため、 ボードごとのversionは持たない)。groupId
 * は必須(個人戦のMatchと同じくグループに帰属する)。
 */
public record TeamMatch(
  ...
  public static TeamMatch pairOf(
    ...
    List<BoardResult> boards =
        IntStream.rangeClosed(1, teamSize).mapToObj(BoardResult::unplayed).toList();
    return new TeamMatch(
        TeamMatchId.generate(),
        roundNumber,
        tableNumber,
        team1Id,
        team2Id,
  ...
  public static TeamMatch byeOf(int roundNumber, int tableNumber, TeamId team1Id, GroupId groupId) {
    return new TeamMatch(
        TeamMatchId.generate(),
        roundNumber,
        tableNumber,
   ...
   * ボード点数の合計そのものではなく、両チームのboardPointsForを比較して導出する (05_swiss_pairing_algorithm.md
   * §5.3)。全ボード決着前はNONE扱いで0を返す
   */
  ...
  public TeamMatch withBoardResults(List<MatchResult> newResults) {
    ...
    List<BoardResult> updated =
        IntStream.range(0, boardResults.size())
  ...
  public TeamMatch withReportedBoardResults(MatchSide side, List<MatchResult> claimed) {
    ...
    List<BoardResult> updated =
        IntStream.range(0, boardResults.size())
            ...
            .toList();
    boolean anyNewlyDecided =
        IntStream.range(0, boardResults.size())
            .anyMatch(
                i ->
                    !boardResults.get(i).result().isDecided()
