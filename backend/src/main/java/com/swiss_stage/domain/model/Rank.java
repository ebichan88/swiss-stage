...
/**
 * 棋力(段級位)。囲碁・将棋共通の29段階。最弱は20級、最強は9段。 1級の次は初段(DAN_1)であり「1段」は存在しない。 (仕様:
 * .claude/01_development_docs/05_swiss_pairing_algorithm.md §1.1)
 *
 * <p>強さの比較は宣言順(ordinal)ではなく sortOrder(小さいほど強い。段=負、級=正)で行う。 将来定数を追加・並べ替えしても強さの定義が壊れないようにするため。 宣言順と
 * sortOrder の整合はテストで検証している。
 */
public enum Rank {
  KYU_20("20級", 20),
  KYU_19("19級", 19),
  ...
  /** 表示名("3級"・"初段"・"2段" 等。"1段" は初段の別表記として許容)からのパース。 CSVインポートで使用する。未知の表記は empty(呼び出し側で行エラーにする)。 */
  public static Optional<Rank> fromDisplayName(String text) {
    ...
    return Arrays.stream(values()).filter(r -> r.displayName.equals(normalized)).findFirst();
  }
}
