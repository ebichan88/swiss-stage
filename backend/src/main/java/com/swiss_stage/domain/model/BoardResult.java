...
/**
 * 主将戦・副将戦…の1ボード分の結果(値オブジェクト。TeamMatchに埋め込み)。 resultはteam1視点で既存MatchResultを再利用する(NONE=未入力。BYEは不可)。
 * team1ReportedResult/team2ReportedResultは共有トークン経由の自己申告(NONE=未申告)。
 * 個人戦のMatch.withReportedResultと同じ突き合わせルールをボード単位で独立に適用する (05_swiss_pairing_algorithm.md
 * §5.4)。MatchSide.PLAYER1/PLAYER2をteam1/team2に読み替える。
 */
