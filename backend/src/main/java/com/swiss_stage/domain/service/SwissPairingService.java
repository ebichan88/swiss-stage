...
/**
 * スイス方式マッチング(個人戦)。バックトラック探索の本体は{@link PairingEngine}に共通化し、 このクラスは Participant/Match
 * をエンジンの汎用形に変換するアダプタに徹する (団体戦は {@code TeamSwissPairingService} が同じ PairingEngine を使う)。
 *
 * <p>制約(仕様: .claude/01_development_docs/05_swiss_pairing_algorithm.md §2):
 *
 * <ul>
 *   <li>[絶対] 再戦禁止 / BYE重複禁止(解がない場合のみ緩和し、結果に記録する)
 *   <li>[優先] 同一所属回避(緩和可)
 * </ul>
 */
public final class SwissPairingService {
  ...
  private PairingResult pairFirstRound(List<Participant> active, PairingOptions options) {
    Comparator<Participant> order =
        Comparator.comparing(Participant::rank, Rank.strongestFirst())
            .thenComparingInt(Participant::entryOrder);
    PairingEngine.PairingOutcome<ParticipantId> outcome =
        engine.pairFirstRound(
  ...
  private static PairingResult toPairingResult(
      PairingEngine.PairingOutcome<ParticipantId> outcome) {
    List<PairingResult.Pair> pairs =
        outcome.pairs().stream()
