 ...
 * ID型に依存しないよう、勝点は呼び出し側が2倍整数(勝=2,分=1,負=0)に変換した {@link DecidedResult} の形で渡す。この点数スケールを用いることで、勝敗の内訳
 * (wins/losses/draws)も点数の値(2/1/0)だけから一意に導出できる。 アルゴリズム本体は{@code
 * StandingCalculator}から移設したもので、挙動は変えていない。 仕様:
 * .claude/01_development_docs/05_swiss_pairing_algorithm.md §3, §5.3, §5.5
 */
final class StandingEngine {
  ...
  <E, ID> List<StandingRow<ID>> calculate(
    ...
    Comparator<StandingRow<ID>> byKeys =
        Comparator.<StandingRow<ID>>comparingInt(StandingRow::points)
            .reversed()
  ...
  private <ID> List<StandingRow<ID>> assignRanks(
    ...
    for (int i = 0; i < sorted.size(); i++) {
      ...
      if (i > 0) {
        ...
        boolean separatedByH2h =
            sameKeys && headToHeadWinner(prev.id(), current.id(), decided).isPresent();
