...
/**
 * スイス方式マッチング(団体戦)。個人戦の{@link SwissPairingService}と同じ {@link PairingEngine} を使い、Team/TeamMatch
 * をエンジンの汎用形に変換するアダプタに徹する。
 *
 * <p>個人戦との差分(仕様: .claude/01_development_docs/05_swiss_pairing_algorithm.md §5.2):
 *
 * <ul>
 *   <li>初回ラウンドのシードなし。エントリー順のみで隣接ペアリングする
 *   <li>チームには「所属」の概念がないため、同一所属回避は適用しない
 *   <li>再戦禁止・BYE重複禁止・バックトラックによる制約緩和は個人戦と同一のルール
 * </ul>
 */
public final class TeamSwissPairingService {
  ...
  public record PairingResult(List<Pair> pairs, TeamId byeId, Set<PairingRelaxation> relaxations) {
