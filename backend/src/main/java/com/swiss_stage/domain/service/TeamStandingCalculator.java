...
/**
 * 順位計算(団体戦)。個人戦の{@link StandingCalculator}と同じ{@link StandingEngine}を使い、 Team/TeamMatch
 * をエンジンの汎用形に変換するアダプタに徹する。入力(チーム+全対局)→ 出力(順位リスト)の純粋関数。
 *
 * <p>勝点・SOS・SOSOS・直接対決・エントリー順によるタイブレークは個人戦と全く同じ基準を
 * チーム単位に適用する。点数の元になる「対局結果」がボード集計値(TeamMatch#pointsFor)で ある点のみが個人戦と異なる(05_swiss_pairing_algorithm.md
 * §5.5)。
 */
public final class TeamStandingCalculator {
  ...
  public List<TeamStanding> calculate(List<Team> teams, List<TeamMatch> matches) {
    List<StandingEngine.DecidedResult<TeamId>> decided =
        matches.stream()
