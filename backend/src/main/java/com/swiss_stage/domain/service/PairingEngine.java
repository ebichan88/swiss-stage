 ...
 * 並び順・勝点・SOS・BYE経験・エントリー順・同一グループ回避判定)はすべて呼び出し側が 関数として渡す。アルゴリズム本体(バックトラック・制約緩和の順序)は {@code
 * SwissPairingService} から移設したもので、挙動は変えていない。 仕様:
 * .claude/01_development_docs/05_swiss_pairing_algorithm.md §2, §5.2
 */
final class PairingEngine {
  ...
  <E, ID> PairingOutcome<ID> pairFirstRound(
    ...
    for (int i = 0; i + 1 < ordered.size(); i += 2) {
      pairs.add(
          new PairingOutcome.Pair<>(idOf.apply(ordered.get(i)), idOf.apply(ordered.get(i + 1))));
    }
  ...
  <E, ID> PairingOutcome<ID> pairLaterRound(
    ...
    toPair.sort(
        Comparator.comparingInt(pointsOf)
            .reversed()
  ...
  /** BYE付与: BYE未経験 かつ 最下位スコアグループ かつ SOS最小。 全員BYE経験済みの場合のみBYE重複を許し、緩和として記録する。 */
  ...
  private <E, ID> List<E> candidatesFor(
    ...
    candidates.sort(
        Comparator.comparingInt(e -> Math.abs(ctx.pointsOf.applyAsInt(e) - firstPoints)));
    return candidates;
  }
