...
public class RoundController {
  ...
  @GetMapping("/rounds")
  public ApiSuccess<List<RoundDto>> list(
  ...
  public ApiSuccess<GeneratedRoundDto> generate(
      CurrentUser user, @PathVariable("tournamentId") String tournamentId) {
    return success(roundService.generateNextRound(PathIds.tournamentId(tournamentId), user.sub()));
  }
  ...
  public ApiSuccess<RoundDto> confirm(
    ...
    return success(
        roundService.confirm(PathIds.tournamentId(tournamentId), roundNumber, user.sub()));
  }
