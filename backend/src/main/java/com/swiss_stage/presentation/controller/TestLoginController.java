...
/** 開発・テスト用の仮ログイン(Google OAuth2はPhase 5)。 local / test プロファイル限定。本番プロファイルではエンドポイント自体が存在しない。 */
@RestController
...
public class TestLoginController {
  ...
  public record TestLoginRequest(@Size(max = 100) String sub, @Size(max = 100) String name) {}
  ...
  public ResponseEntity<ApiSuccess<AuthController.MeDto>> testLogin(
    ...
    String sub =
        request == null || request.sub() == null || request.sub().isBlank()
            ? "test-user"
            : request.sub();
    String name =
