...
public class WebMvcConfig implements WebMvcConfigurer {
  ...
  /**
   * SPA配信(04_react_router_patterns.md §5、14_performance_optimization.md §4)。 フロントエンドのビルド成果物は
   * classpath:/static/ に配置される(11_cicd_design.md)。
   *
   * <ul>
   *   <li>/assets/**(Viteのハッシュ付きファイル)… 1年 + immutable
   *   <li>その他 … no-cache。存在しないパスは index.html へフォールバックし、 /tournaments/xxx 等への直アクセスでもSPAが起動する
   *   <li>/api/** はフォールバックしない(未知のAPIパスは404のまま)
   * </ul>
   *
   * <p>デフォルトの静的リソースマッピングは application.yml (spring.web.resources.add-mappings=false)で無効化している。
   */
  @Override
