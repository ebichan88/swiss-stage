...
/**
 * 運営者セッションJWTの発行・検証とCookie組み立て(13_security_design.md §2)。 HttpOnly +
 * SameSite=Lax。Secure属性はローカル開発(http)のみ無効化できる。 Google OAuth2での発行フローは Phase 5 で追加する。
 */
@Component
public class JwtSessionSupport {
  ...
  public JwtSessionSupport(
    ...
    if (secret == null || secret.length() < 32) {
      throw new IllegalStateException("app.auth.jwt-secret(環境変数JWT_SECRET)は32文字以上で設定してください");
    }
