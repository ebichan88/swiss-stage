...
class TeamRosterValidationServiceTest {
  ...
  void 開始時検証OK() {
    ...
    assertThatCode(() -> service.validateForStart(List.of(team), 3)).doesNotThrowAnyException();
  }
  ...
  void 棄権チームは検証対象外() {
    Team incomplete = TeamTestData.team(1).withMember(TeamMember.create("主将", null, 1)).withdraw();
