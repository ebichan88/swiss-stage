...
/**
 * 団体戦スイス方式マッチングのプロパティテスト。 16〜60チーム・5回戦をシミュレートし、絶対制約(再戦なし・BYE重複なし・全員毎ラウンド1回登場)を
...
class TeamSwissPairingPropertyTest {
  ...
  void 全ラウンドを通して絶対制約が守られる(
      @ForAll @IntRange(min = 16, max = 60) int teamCount, @ForAll long resultSeed) {
    ...
    for (int round = 1; round <= ROUNDS; round++) {
      ...
      result
          .pairs()
          .forEach(
              pair -> playedPairs.add(Set.of(pair.firstId().value(), pair.secondId().value())));
