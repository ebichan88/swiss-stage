...
class GroupApiTest extends ApiContractTestSupport {
    ...
    MvcResult result =
        performApi(
                post("/api/v1/tournaments")
                    ...
                    .content(
                        "{\"name\":\"グループテスト大会\",\"gameType\":\"GO\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":3}"))
  ...
  @Test
  @DisplayName(
      "GRP-AC-001,GRP-AC-002,GRP-AC-003,GRP-AC-004,GRP-AC-005: "
  ...
  void 自動振り分けと個別調整() throws Exception {
    ...
    // 個別調整: 三郎をBへ
    performApi(
            patch(participantsPath() + "/" + ids.get(2))
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void 自動振り分けは棄権者の割当を維持() throws Exception {
    ...
    performApi(
            patch(participantsPath() + "/" + withdrawnId)
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void 開始時検証() throws Exception {
    ...
    performApi(post(base() + "/start").cookie(ownerCookie())).andExpect(status().isConflict());
    ...
    performApi(post(groupsPath() + "/auto-assign").cookie(ownerCookie()))
        .andExpect(status().isConflict());
    performApi(
            patch(participantsPath() + "/" + ids.get(0))
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"groupId\":\"" + groupB + "\"}"))
        .andExpect(status().isConflict());
  ...
  void グループ大会の一巡() throws Exception {
    ...
    List<JsonNode> groupAMatches =
        matches.stream().filter(m -> m.path("group").path("id").asText().equals(groupA)).toList();
    List<JsonNode> groupBMatches =
        matches.stream().filter(m -> m.path("group").path("id").asText().equals(groupB)).toList();
    assertThat(groupAMatches).extracting(m -> m.path("tableNumber").asInt()).containsExactly(1, 2);
    assertThat(groupBMatches).extracting(m -> m.path("tableNumber").asInt()).containsExactly(1);
    assertThat(groupAMatches).extracting(m -> m.path("result").asText()).contains("BYE");
    assertThat(groupBMatches.getFirst().path("result").asText()).isNotEqualTo("BYE");
    assertThat(matches)
        .allSatisfy(m -> assertThat(m.path("group").path("name").asText()).isIn("A", "B"));
