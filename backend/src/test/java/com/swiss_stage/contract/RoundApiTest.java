...
class RoundApiTest extends ApiContractTestSupport {
    ...
    MvcResult result =
        performApi(
                post("/api/v1/tournaments")
                    ...
                    .content(
                        "{\"name\":\"ラウンドテスト大会\",\"gameType\":\"GO\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":3}"))
  ...
  @Test
  @DisplayName(
      "RND-AC-001,RND-AC-002,RND-AC-003,RND-AC-004,RND-AC-005,RND-AC-006,RND-AC-007: "
  ...
  @Test
  @DisplayName("RND-AC-012: 片方のみ申告・申告不一致で結果が未確定のままではラウンド確定できない。" + "運営者が結果を確定すれば確定できる")
  void 申告未確定のままではラウンド確定できない() throws Exception {
    ...
    reportSharedResult(
            token, waitingMatch.path("id").asText(), "PLAYER1", "PLAYER1_WIN", waitingVersion)
        .andExpect(status().isOk())
    ...
    reportSharedResult(
            token,
            conflictingMatch.path("id").asText(),
            "PLAYER1",
            "PLAYER1_WIN",
            conflictingVersion)
        .andExpect(status().isOk());
    ...
    performApi(
            put(base() + "/matches/" + waitingMatch.path("id").asText() + "/result")
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"result\":\"PLAYER1_WIN\",\"version\":" + waitingVersion + "}"))
        .andExpect(status().isOk());
    performApi(
            put(base() + "/matches/" + conflictingMatch.path("id").asText() + "/result")
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  private ResultActions reportSharedResult(
      ...
      throws Exception {
    return performApi(
        put("/api/v1/shared/" + token + "/matches/" + matchId + "/result")
            .contentType(MediaType.APPLICATION_JSON)
            .content(
                "{\"reportedBy\":\""
                    + reportedBy
