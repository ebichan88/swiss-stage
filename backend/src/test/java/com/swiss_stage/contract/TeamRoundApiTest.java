...
class TeamRoundApiTest extends ApiContractTestSupport {
  ...
  @Test
  @DisplayName(
      "TEAM-AC-012,TEAM-AC-013,TEAM-AC-016,TEAM-AC-017: "
  ...
  @Test
  @DisplayName("TEAM-AC-014,TEAM-AC-015: 運営者が全ボード結果をまとめて直接確定でき、" + "未着手の対局が残るラウンドは確定できない")
  ...
  private void inputAllBoardsWin(JsonNode match, String result) throws Exception {
    performApi(
            put(base() + "/team-matches/" + match.path("id").asText() + "/result")
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"boardResults\":[\""
