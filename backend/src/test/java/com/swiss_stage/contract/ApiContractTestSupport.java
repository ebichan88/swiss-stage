...
public abstract class ApiContractTestSupport {
      ...
      OpenApiInteractionValidator.createForSpecificationUrl("/openapi.yaml")
          ...
          .withWhitelist(
              ValidationErrorsWhitelist.create()
                  .withRule(
                      "multipart request body (MockMvc limitation)",
                      WhitelistRules.allOf(
                          WhitelistRules.messageHasKey("validation.request.body.missing"),
                          WhitelistRules.anyOf(
                              WhitelistRules.pathContainsSubstring("/participants/import"),
                              WhitelistRules.pathContainsSubstring("/teams/csv-import")))))
          .build();
  ...
  /**
   * mockMvc.perform + OpenAPIスキーマ検証(schema/openapi.yaml)。 /api/v1 のJSON API呼び出しは原則こちらを使う。次の場合のみ素の
   * mockMvc.perform を使う:
   *
   * <ul>
   *   <li>意図的にスキーマ違反のリクエストを送るテスト(バリデーションエラー系)
   *   <li>SPA配信(HTML)・OAuth2リダイレクトなどスキーマの対象外
   * </ul>
   */
