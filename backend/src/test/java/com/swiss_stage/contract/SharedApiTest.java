...
/** 共有トークン系API(Phase 5)の契約テスト: 発行/再発行・/shared/{token} 集約・トークン経由の結果入力・情報漏えい防止。 */
class SharedApiTest extends ApiContractTestSupport {
    ...
    MvcResult result =
        performApi(
                post("/api/v1/tournaments")
                    ...
                    .content(
                        "{\"name\":\"共有テスト大会\",\"gameType\":\"GO\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":3}"))
  ...
  void トークン発行と再発行() throws Exception {
    ...
    // 未認証は401
    performApi(post(base() + "/share-token/regenerate")).andExpect(status().isUnauthorized());
  ...
  void 共有ページ集約() throws Exception {
    ...
    performApi(post(base() + "/rounds").cookie(ownerCookie())).andExpect(status().isCreated());
    ...
    MvcResult shared =
        performApi(get("/api/v1/shared/" + token))
            ...
            .andExpect(jsonPath("$.data.standings[0].standings[0].participant.name").value("参加 一郎"))
            .andExpect(jsonPath("$.data.standings[0].standings[0].participant.rank").value("DAN_3"))
            .andExpect(jsonPath("$.data.standings[0].standings[0].participant.entryOrder").value(1))
            .andReturn();
  ...
  void トークン経由の結果入力() throws Exception {
    ...
    for (JsonNode m : dataOf(round).path("round").path("matches")) {
      if (!m.path("id").asText().equals(matchId)) {
        performApi(
                put(base() + "/matches/" + m.path("id").asText() + "/result")
                    .cookie(ownerCookie())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        "{\"result\":\"PLAYER1_WIN\",\"version\":"
    ...
    performApi(
            put(base() + "/matches/" + matchId + "/result")
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"result\":\"PLAYER1_WIN\",\"version\":" + (version + 1) + "}"))
        .andExpect(status().isOk());

    performApi(post(base() + "/rounds/1/confirm").cookie(ownerCookie())).andExpect(status().isOk());
  ...
  @Test
  @DisplayName(
      "SHR-AC-011,SHR-AC-012: 片方のみの申告では確定せず、両者の申告が一致すると自動確定する。" + "一致しない間は結果を確定せず双方の申告が参照できる")
  ...
  void キャッシュevict() throws Exception {
    ...
    JsonNode match = dataOf(round).path("round").path("matches").get(0);
    performApi(
            put(base() + "/matches/" + match.path("id").asText() + "/result")
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"result\":\"PLAYER1_WIN\",\"version\":"
    ...
    MvcResult participants =
        performApi(get(base() + "/participants").cookie(ownerCookie()))
            .andExpect(status().isOk())
            .andReturn();
    String participantId = dataOf(participants).get(0).path("id").asText();
    performApi(
            patch(base() + "/participants/" + participantId)
                .cookie(ownerCookie())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"name\":\"改名 太郎\"}"))
        .andExpect(status().isOk());
    MvcResult shared =
        performApi(get("/api/v1/shared/" + token)).andExpect(status().isOk()).andReturn();
  ...
  private String regenerateToken() throws Exception {
    MvcResult result =
        performApi(post(base() + "/share-token/regenerate").cookie(ownerCookie()))
            .andExpect(status().isOk())
  ...
  private void setVisibility(String visibility) throws Exception {
    performApi(
            patch(base())
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"visibility\":\"" + visibility + "\",\"version\":" + currentVersion() + "}"))
        .andExpect(status().isOk());
  }
  ...
  private long currentVersion() throws Exception {
    MvcResult result =
        performApi(get(base()).cookie(ownerCookie())).andExpect(status().isOk()).andReturn();
