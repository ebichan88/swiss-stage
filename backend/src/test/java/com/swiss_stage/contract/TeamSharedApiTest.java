...
class TeamSharedApiTest extends ApiContractTestSupport {
  ...
  void setUp() throws Exception {
    ...
    for (String name : new String[] {"Aチーム", "Bチーム"}) {
      ...
      for (int position = 1; position <= 3; position++) {
        performApi(
                post(base() + "/teams/" + teamId + "/members")
                    .cookie(ownerCookie())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        "{\"name\":\"メンバー" + position + "\",\"boardPosition\":" + position + "}"))
            .andExpect(status().isCreated());
      }
    }
  }
  ...
  @Test
  @DisplayName("共有ページ集約は団体戦ではteamRounds/teamStandingsを返し、rounds/standingsはnullになる。" + "個人名を含まない")
  void 共有ページ集約はチーム形式() throws Exception {
    ...
    performApi(post(base() + "/team-rounds").cookie(ownerCookie())).andExpect(status().isCreated());
  ...
  void ボード単位の自己申告() throws Exception {
    ...
    performApi(
            reportTeamResult(
                ...
                matchId,
                "PLAYER1",
                "[\"PLAYER1_WIN\",\"PLAYER1_WIN\",\"PLAYER2_WIN\"]",
                version))
        .andExpect(status().isOk())
    ...
    performApi(
            reportTeamResult(
                ...
                matchId,
                "PLAYER2",
                "[\"PLAYER1_WIN\",\"PLAYER1_WIN\",\"PLAYER1_WIN\"]",
                version + 1))
        .andExpect(status().isOk())
  ...
  void 運営者確定は自己申告で上書きされない() throws Exception {
    ...
    performApi(
            put(base() + "/team-matches/" + matchId + "/result")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"boardResults\":[\"DRAW\",\"NONE\",\"NONE\"],\"version\":" + version + "}"))
        .andExpect(status().isOk());
  ...
  void 自動確定は自己申告の変更で上書きされない() throws Exception {
    ...
    performApi(
            reportTeamResult(
                ...
                matchId,
                "PLAYER1",
                "[\"PLAYER2_WIN\",\"PLAYER1_WIN\",\"PLAYER1_WIN\"]",
                version + 2))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.boardResults[0].result").value("PLAYER1_WIN"));
  }
