...
/**
 * SPA配信とフォールバックの契約(04_react_router_patterns.md §5、14_performance_optimization.md §4)。 テスト用の
 * index.html / assets は src/test/resources/static に置いてある。
 */
class SpaFallbackApiTest extends ApiContractTestSupport {
    ...
    for (String path :
      ...
      MvcResult result =
          mockMvc
              ...
              .andReturn();
      assertThat(result.getResponse().getContentAsString()).contains("spa-fallback-test-marker");
    }
  }
  ...
  void ルートアクセス() throws Exception {
    mockMvc
        ...
        .andExpect(status().isOk())
        .andExpect(
            content().string(org.hamcrest.Matchers.containsString("spa-fallback-test-marker")));
  ...
  void アセットの長期キャッシュ() throws Exception {
    mockMvc
        .perform(get("/assets/app-test.js"))
        .andExpect(status().isOk())
  ...
  void APIは対象外() throws Exception {
    MvcResult result =
        mockMvc
            .perform(get("/api/v1/unknown-endpoint"))
