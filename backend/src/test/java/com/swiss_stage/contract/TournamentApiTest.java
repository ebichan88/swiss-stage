...
class TournamentApiTest extends ApiContractTestSupport {
    ...
    MvcResult result =
        performApi(
                post("/api/v1/tournaments")
                    ...
                    .content(
                        "{\"name\":\"契約テスト大会\",\"gameType\":\"GO\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":3}"))
  ...
  void 作成() throws Exception {
    performApi(
            post("/api/v1/tournaments")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"name\":\"新規大会\",\"gameType\":\"SHOGI\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":5}"))
        .andExpect(status().isCreated())
    ...
    mockMvc
        .perform(
            post("/api/v1/tournaments")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"name\":\"\",\"gameType\":\"GO\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":0}"))
    ...
    mockMvc
        .perform(
            post("/api/v1/tournaments")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"name\":\"上限超過大会\",\"gameType\":\"GO\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":9}"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.error.code").value("VALIDATION_ERROR"));
  }
  ...
  void ラウンド数上限ちょうど() throws Exception {
    performApi(
            post("/api/v1/tournaments")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"name\":\"上限大会\",\"gameType\":\"GO\",\"competitionType\":\"INDIVIDUAL\",\"totalRounds\":8}"))
        .andExpect(status().isCreated())
  ...
  void 取得と認可() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/tournaments/not-a-ulid").cookie(ownerCookie()))
        .andExpect(status().isNotFound());

    // パストラバーサルはSpring SecurityのHttpFirewallが400で拒否する(Phase 5〜)
    mockMvc
        .perform(get("/api/v1/tournaments/../etc").cookie(ownerCookie()))
        .andExpect(status().isBadRequest());
  }
