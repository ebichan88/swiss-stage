...
/**
 * DynamoDB Localに対するリポジトリ統合テストの共通基盤 (.claude/03_library_docs/03_dynamodb_local_testing.md)。
 *
 * <p>テーブルは消さず、テストごとに一意のULIDでデータを分離する。 テーブル作成は冪等(既存ならスキップ)なので、ローカル・CIのどちらでも動く。
 */
@SpringBootTest
@ActiveProfiles("test")
public abstract class DynamoDbRepositoryTestSupport {
  ...
  @Autowired private DynamoDbClient client;
