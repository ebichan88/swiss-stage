...
class ArchitectureTest {
  ...
  static final ArchRule domainは外部フレームワークに依存しない =
      ArchRuleDefinition.noClasses()
          .that()
          .resideInAPackage("com.swiss_stage.domain..")
          .should()
          .dependOnClassesThat()
          .resideInAnyPackage("org.springframework..", "software.amazon.awssdk..", "jakarta..");
  ...
  /**
   * リクエスト引数アノテーションは名前を明示する。 名前を省略するとコンパイル時の -parameters フラグに依存し、 IDE(Eclipse
   * JDT)ビルドで起動した場合に実行時エラーになるため。
   */
  @ArchTest
