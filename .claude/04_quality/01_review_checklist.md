...
- **FE-5**: 新規画面・一覧表示に**ローディング / エラー(再試行付き) / 空状態(0件)の3状態**が実装されているか(`10_frontend_design.md` §2④)。データ取得があるのに正常系しか描画していない差分は指摘する
- **FE-6**: スマホ幅(375px)で破綻しない実装か — 固定px幅、`overflow` 対策のないテーブル、横スクロールを誘発するレイアウトがないか(`04_layout_system.md` §1、スマホファースト)
...
- **TEST-5**: 境界を持つ入力(人数・ラウンド数・チーム制・文字数)のテストに、境界の**両側**(on/off point)があるか(`09_test_strategy.md` §2.5)。片側だけのテストは「境界がずれても気づけない」ので指摘する
- **TEST-6**: 複数条件の組み合わせを含む変更で、**どの因子・水準を選んだかの根拠**が `@DisplayName` かコメントに残っているか(同 §2.5 のペアワイズ因子表)。根拠がないと網羅の妥当性を後から検証できない
