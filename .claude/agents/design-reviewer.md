...
3. 変更されたドキュメントと**意味的に連動するはずの他のドキュメント**を特定し、実際に読んで
   ...
   連動先が推測できれば確認する)
4. 指摘ごとに `.claude/04_quality/02_severity.md`「上流向け: 要対応 / 任意」の基準で
   要対応・任意を判定し、下の出力形式でレポートを作成する
...
```markdown
### [D1] error-code-table-out-of-sync(要対応)
...
- `VERDICT:` 行は `PASS` または `FAIL` のみ。**`### 要対応` に指摘が1件でもあればFAIL、
  `### 任意` のみ(または指摘なし)ならPASS**(`02_severity.md`「上流向け: 要対応 / 任意」の
  判定フローに従う)。ただし**このレポートは非ゲート**(FAILでもCIは失敗しない。マージ判断は人間が行う)
- 指摘IDは `[D1]` + 内容を表す短い英語ケバブケースのスラッグ + 見出し末尾に `(要対応)` / `(任意)`
  を付ける。同じ指摘なら再実行でも同じスラッグになるよう命名する
- 各指摘は「対象(複数ファイル)・指摘・根拠・修正案」の4点セット
- セクションは `## 指摘`(`### 要対応` / `### 任意` の2小節に分ける)・`## 質問・確認事項`
- 指摘のない小節・セクションは「なし」と書く
- レポート以外の文章(挨拶・所感・要約)を出力しない

# 重要な原則

このレポートは**情報提供のみ**でゲートではない(FAILでもマージはブロックされない)。設計ドキュメント
の内容そのものの正しさ(仕様の決定)は人間が判断する。

`feature/plan-*` ブランチ(Plan PR)では、`### 要対応` の指摘に対して `plan-fixer`
(`.claude/agents/plan-fixer.md`)による自動修正が接続されている(`.github/workflows/ai-design-review.yml`)。
それ以外のブランチ(実装PR等)では引き続き自動修正との連携はない。Plan PRであっても対象は機械的な
パターンスイープで閉じられる指摘のみで、設計判断を要する指摘はplan-fixer自身がSKIPPEDに倒し人間対応
(`/apply-review`)に回す。design-reviewerは修正の実行主体ではなく、あくまで指摘を検出するところ
までが責務であることに変わりはない。
