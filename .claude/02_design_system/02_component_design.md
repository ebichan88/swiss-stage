...
- **Tooltipを使わない**: 紙にホバー操作は存在しない。戦績一覧表(印刷)の相手列はNo.のみとし、同じ表の左端にNo.↔氏名の対応があるため引ける(日本棋院の大会結果表と同じ自己参照構造)。SOS/SOSOSの説明(`TableHeaderTooltip`相当)は表の下に凡例1行で足す
...
- 個人名の掲載可否は帳票ごとに異なる(`04_screen_transition_design.md` §5-2)。団体戦の対局カード・戦績一覧表(印刷)は
  チーム名+ボード役割のみで、参加者名簿(運営専用)のみメンバー氏名を出してよい
