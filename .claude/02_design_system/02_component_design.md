...
- **ヘッダー帯**: 大会名・状態バッジ・現在ラウンドを `primary.light` の帯(角丸・内側余白)で囲み、
  ページ上部に基調色を置く。運営者画面の見出しは現時点では対象外(共有ページ固有)。
  運営者管理画面の共通レイアウト(`TournamentLayout`)の見出し行への適用は
  `.claude/07_plans/04_design_system_rollout.md` で計画中(実装後にこの記述を更新する)
