...
| TRN-AC-019 | P2 | 個別admin画面のフォーム(ParticipantFormDialog/TeamFormDialog/SettingsPage)は意匠変更後も、入力エラー時にhelperTextのエラーメッセージが表示され、エラーの入力欄にaria-describedbyで関連付けられる | done | ParticipantFormDialog.test, SettingsPage.test(Vitest) |
