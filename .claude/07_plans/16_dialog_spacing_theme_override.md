...
- **レスポンシブ**: 375px〜デスクトップの両方で同じpadding値を適用する(運営者管理画面向けの
  モーダルであり、`00_basic_design.md` §4 の優先度上も「4. 運営者管理画面」= MUI標準に寄せてよい
  区分のため、ブレークポイントごとの出し分けは行わない)
