...
- 訪問者が トップページの「運営者ログイン」ボタンを押すと ログイン画面(`/login`)に遷移する
  (既存の `component={Link} to={paths.login}` のまま、変更しない)
...
- [x] `CLAUDE.md` 落とし穴#18 — 「新規画面(0→1)のみ」の記述をADR 12の決定に合わせて一般化し、
      `06_adr/12_story_first_existing_page_placeholder.md` への参照を追加(AI Design Review [D1]対応)
- [x] `.claude/00_project/04_development_process.md` §2・§4(追加分) — Plan PR内でADRを
      `Accepted`にする例外の場合、対象の設計ドキュメントもPlan PR内で更新してよい旨をトリガー表
      直後の記述にも明記。ADRの部分補正(Superseded化しない場合)の運用規定を追記
      (AI Design Review [D2][D3]対応)

上記の更新は、ADR 12が「Plan PRの作り方」自体を決定しており、その決定を前提にした
...
- 本Plan PRのCI実行で、`.github/workflows/vrt.yml` の「新規ストーリー(ベースライン未確定)を検出」
  ステップに不具合(シンタックスエラー→検出0件→dubious ownershipエラー、の3段階)が見つかった。
  本プランの範囲外(#170/#172の実装バグ)のため、Issue #175を起票し別PR(#176・#177)で修正・
  検証・mainへのマージを完了した。この計画の検証目的(#170メカニズムの実地確認)の一環で見つかり、
  結果としてPlan PRブランチへの`.stories.tsx`追加が実際にCIを通じて機能することまで確認できた
