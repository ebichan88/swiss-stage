...
| 無効な招待トークン | 403 | `INVALID_INVITE_TOKEN` | この招待リンクは無効になっています。運営者に確認してください | INFO |
| リソース未存在 | 404 | `TOURNAMENT_NOT_FOUND` / `PARTICIPANT_NOT_FOUND` / `ROUND_NOT_FOUND` / `MATCH_NOT_FOUND` / `GROUP_NOT_FOUND` / `TEAM_NOT_FOUND` / `TEAM_MEMBER_NOT_FOUND` / `TEAM_MATCH_NOT_FOUND` / `TOURNAMENT_MEMBER_NOT_FOUND` / `NOT_FOUND`(汎用) | 大会が見つかりません | INFO |
