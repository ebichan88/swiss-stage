...
- ページは本番と同じ経路(直import・propsなし・hooks経由)でレンダリングする。
  ストーリー専用のダミー画面は作らない
- **新規画面(0→1)・既存画面の大きなレイアウト変更の例外**: Plan PR時点で本物のページ
  (`src/pages/XxxPage.tsx`)が対象の新レイアウトをまだ反映していない(新規画面なら未実装、
  既存画面なら旧レイアウトのまま)間は、ストーリーファイル内にインラインのプレースホルダー実装を
  置いてよい(上記「ストーリー専用ダミー画面は作らない」原則のPlan PR時点での例外)。実装PRで
  本物のページをこのプレースホルダーに合わせて書き換え、ストーリーを実importへ書き換える
  (`.claude/06_adr/12_story_first_existing_page_placeholder.md`)
