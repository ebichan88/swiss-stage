...
3. 申告が一致しない場合は確定せず、双方の申告内容(`player1ReportedResult`/`player2ReportedResult`)を共有ページ・
   運営者のラウンド管理画面の両方に表示する(運営者が内容を確認し、`InputResultRequest` で直接確定して解決する)
4. **運営者が `InputResultRequest` で直接確定した結果は最終権威とし、その後の参加者の自己申告(一致・不一致問わず)では
   上書きしない**。申告自体は記録として残るが `result` には影響しない
5. ラウンド確定(`POST /rounds/{n}/confirm`)がブロックされるのは、運営者・参加者のいずれも一切触れていない対局が
   残っている場合のみ。片方のみ申告・申告不一致の対局は警告表示のみでブロックしない(運営者の裁量で確定できる。
   進行の柔軟性を優先し、正確性は上記1〜4の突き合わせ・運営者の最終確認で担保する)
