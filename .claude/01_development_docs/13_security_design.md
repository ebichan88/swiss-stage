# セキュリティ設計書

## 1. 基本方針

- 個人情報(氏名・所属企業)を扱うため、**個人情報保護をプロダクト品質の一部**として扱う
- 保持する個人情報は必要最小限(氏名・所属・段級位のみ。メールアドレスは運営者のGoogleアカウントのみ)
- 「見せる範囲」は常に運営者がコントロールできる

---

## 2. 認証

### 運営者: Google OAuth2 + JWT

- Spring Security `oauth2-client` で Google OAuth2 認証
- 認証成功後、自前のJWT(HS256)を発行し **HttpOnly + Secure + SameSite=Lax の Cookie** に格納
  - localStorage には保存しない(XSS耐性)
- JWTクレーム: `sub`(Google sub), `name`, `exp`(24時間)
- `JWT_SECRET` は32文字以上、環境変数で注入

### 参加者: 共有トークン

- 大会ごとに1つの共有トークン(`ShareToken`)を発行。URLに含める(`/s/{token}`)
- トークンは `SecureRandom` で32文字以上(URL-safe Base64)。**推測可能な連番・短いトークン禁止**
- 運営者はいつでも再発行(旧トークン即時無効化)できる
- トークンでできる操作は「閲覧」+「結果の自己申告(大会設定 `resultInputEnabled` で許可時のみ)」に限定
- トークン経由のAPIは `/api/v1/shared/{token}` 系のみ。無効・不明・非公開(PRIVATE)はすべて同一の403 `INVALID_SHARE_TOKEN` で返す(存在を漏らさない)

### 結果確定の運用ルール(自己申告)

共有トークンは大会に1つで参加者個人には紐付かない(閲覧者を個別に認証しない)ため、片方の入力だけで
結果を即確定する方式は「入力ミス」「対戦相手が勝手に自分に有利な結果を送信する」といった不正確さ・悪意に弱い。
これを緩和するため、トークン経由の結果入力は次の運用ルールで**自己申告の突き合わせ**にする:

1. 参加者は結果を送信する際、「自分がplayer1/player2のどちらか」(`MatchSide`)を明示して申告する
   (`ReportMatchResultRequest`。運営者の直接確定用 `InputResultRequest` とはスキーマを分ける)
2. 片方のみの申告では対局結果(`Match.result`)は確定しない(`NONE` のまま)。両者の申告が一致した時点で自動確定する
3. 申告が一致しない場合は確定せず、双方が具体的に何を申告したか(`player1ReportedResult`/`player2ReportedResult`
   の値そのもの。「不一致」という状態語だけでなく誰が勝ち/負け/引き分け/両者負けを申告したか)を共有ページ・
   運営者のラウンド管理画面の両方に表示する(運営者が内容を確認し、`InputResultRequest` で直接確定して解決する)
4. **確定済みの結果(`InputResultRequest` による運営者の直接確定・両者の自己申告一致による自動確定のいずれも)は
   最終権威とし、その後の参加者の自己申告(一致・不一致問わず)では上書きしない**。申告自体は記録として残るが
   `result` には影響しない(自動確定後に片方が申告を変えても、結果がサイレントに書き換わったり未確定に
   巻き戻ったりしない)
5. 確定済みの結果と、確定後に変化した自己申告(`player1ReportedResult`/`player2ReportedResult`)が食い違う場合、
   3. と同様に具体的な申告内容を運営者のラウンド管理画面・参加者の共有ページの両方で検知・表示する。ブロックは
   せず、運営者が内容を確認して必要なら `InputResultRequest` で結果を修正する運用とする(この検知はフロントエンド
   が既存のレスポンスフィールド(`result`/`player1ReportedResult`/`player2ReportedResult`)から算出し、APIの
   追加フィールドは持たない)
6. ラウンド確定(`POST /rounds/{n}/confirm`)は、結果(`result`)が未確定(NONE)の対局が1件でも残っていると
   ブロックされる(409)。片方のみ申告・申告不一致の対局も`result`はNONEのままのため同様にブロックされ、
   運営者が申告内容を確認したうえで`InputResultRequest`により結果を直接確定して初めて確定できる。確定後は
   編集不可になり、`result`がNONEのままだと順位計算からその対局が恒久的に除外されてしまうため、進行の柔軟性より
   正確性を優先する

この方式でも、共有リンクを持つ第三者が虚偽の申告を送ること自体は防げない(なりすまし対策ではない)。
あくまで「片方の入力ミス・一方的な虚偽申告」を両者の突き合わせで検知し、運営者の確認を促す仕組みである。
参加者ごとの個別リンク発行(本人性の担保)は将来検討事項とし、MVPでは採用しない。

団体戦(`competitionType=TEAM`)では上記1〜6を**ボード(主将戦・副将戦…)単位**に適用する。参加者は「どちらのチームか」を
選択したうえで全ボード分の申告を1回の送信でまとめて行うが、確定判定・不整合検知・最終権威の扱いはボードごとに独立して
行う(1ボードの不一致が他のボードの確定を妨げない)。詳細は `05_swiss_pairing_algorithm.md` §5.4 を参照。

---

## 3. 認可

| 操作 | OWNER | MAINTAINER | 運営者(無関係) | 共有トークン | 未認証 |
|------|:---:|:---:|:---:|:---:|:---:|
| 大会の作成 | ○ | - | - | × | × |
| 大会の閲覧・参加者管理・ラウンド進行・結果入力・順位表・帳票印刷 | ○ | ○ | × | ○(閲覧・自己申告のみ) | × |
| 大会設定(名称・公開範囲等)・削除・共有トークン再発行・共同管理者/招待の管理 | ○ | × | × | × | × |
| 結果入力(運営者は直接確定・共有トークンは自己申告) | ○ | ○ | × | △(設定による) | × |

- 大会に対する役割は`TournamentAccessSupport`が`loadOwner`(OWNER専用)・`loadMember`(OWNER・MAINTAINER共通)の2メソッドに集約して検証する(application層で必ずチェック。presentation層のチェックだけに頼らない)。役割はordinal比較せず`TournamentRole`の明示的な述語で判定する(CLAUDE.md #13)
- **404と403の使い分け**: メンバーでない(OWNERでもMAINTAINERでもない)場合は他人の大会の存在を漏らさないため404 `TOURNAMENT_NOT_FOUND`。メンバーではあるが権限が足りない(MAINTAINERがOWNER専用操作を呼んだ)場合は403 `FORBIDDEN`(「自分が所属している大会である」という情報以上は増えないため存在秘匿は保たれる)
- 招待リンクの発行・受諾によるMAINTAINER追加は`14_tournament_collaboration.md`を参照(別PRで実装)

---

## 4. 入力検証

- Bean Validation(JSR 380)を全DTOに適用: 文字数上限(大会名100文字、氏名50文字等)、必須、形式
- CSVインポート: 行数上限(500行)、ファイルサイズ上限(1MB)、拡張子・Content-Type検証
- フロントでも同等のバリデーションを行うが、**信頼するのはバックエンドの検証のみ**

---

## 5. Webセキュリティ対策

| 脅威 | 対策 |
|------|------|
| XSS | Reactのデフォルトエスケープに依存。`dangerouslySetInnerHTML` 禁止。CSPヘッダー設定 |
| CSRF | 認証CookieがSameSite=Lax + 更新系はJSON API(CORSで防御)。Spring SecurityのCSRF設定はAPI構成に合わせ調整 |
| CORS | 本番は自ドメインのみ許可。`*` 禁止 |
| インジェクション | DynamoDBのため SQLインジェクションなし。ただしキー組み立て時のユーザー入力混入を禁止(IDはULID検証必須) |
| レート制限 | 結果入力・トークンアクセスにIPベースの簡易レート制限(bucket4j)。ブルートフォースによるトークン探索対策 |
| 依存脆弱性 | Dependabot自動更新 + `pnpm audit` / `gradlew dependencyCheckAnalyze` を月次実行 |
| セキュリティヘッダー | `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin` |

---

## 6. 個人情報保護の実装ルール

1. **ログに個人名・所属を出力しない**(IDのみ。`06_error_handling_design.md` 参照)
2. **大会削除は物理削除**: 主催者が削除したら参加者情報も含め完全に消す
3. 公開範囲 `PRIVATE`(運営者のみ)/ `TOKEN`(URL知っている人)をAPIレベルで強制
4. 共有トークン経由のレスポンスに `shareToken`・`ownerSub` を含めない(DTO出し分け)
5. 利用規約・プライバシーポリシーページを公開前に用意する(保持情報・目的・削除方法を明記)

---

## 7. インフラセキュリティ

- ALBでSSL/TLS終端(ACM証明書)。HTTP→HTTPSリダイレクト必須
- EC2セキュリティグループ: ALBからの8080のみ許可。SSHは自宅IPのみ(将来はSSM Session Manager化)
- IAM: EC2ロールはDynamoDBの対象テーブルのみ許可(最小権限)。アクセスキーをコードに書かない
- 秘密情報: 環境変数ファイル(600)で管理、リポジトリにコミットしない。`.gitignore` に `.env*` を必ず含める
