...
削減対象は「手書きで名簿を作る」「結果を紙から紙へ転記する」といった**手作業**であり、紙媒体そのものの排除ではない。
年配の参加者や子供などスマホを持たない層は必ず一定数いるため、Swiss Stageが持つデータから対局カード・対戦結果表・
参加者名簿を印刷できるようにする(帳票印刷、S13)。これはデジタルを正として紙を出力する機能であり、原則には反しない。
