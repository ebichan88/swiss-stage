...
export interface TeamCsvImportDialogProps {
  open: boolean;
  loading: boolean;
...
/** チーム+メンバー一覧のCSVインポート(02_component_design.md §4: D&D+ファイル選択の両対応) */
export function TeamCsvImportDialog({
  open,
  loading,
  error,
  onImport,
  onClose,
}: TeamCsvImportDialogProps) {
    ...
    >
      ...
      <DialogContent>
        <Typography variant="body2" color="text.secondary" gutterBottom>
          ...
          まとまります。ポジションは「主将・副将・三将…」(チーム制の人数まで)、空欄は補欠です。
          グループ分けする場合は5列目に「グループ」列を追加できます。文字コードは UTF-8 / Shift_JIS
