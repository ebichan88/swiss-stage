...
test('CP4-1: 結果未入力の対局があるとラウンドを確定できず警告が表示される', async ({ page }) => {
...
test('CP4-2: 2つのブラウザで同じ対局に別の結果を入力すると後勝ちにならず409になる', async ({
  page,
  browser,
}) => {
  await loginAsOrganizer(page);
...
test('CP4-3: 再発行で無効化した共有トークンはエラーページになる', async ({ page, browser }) => {
  await loginAsOrganizer(page);
